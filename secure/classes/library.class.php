...
class library
{
	...
	function createNew($name, $nickname, $ils_prefix, $reserveDesk, $url, $contactEmail, $monograph_library_id, $multimedia_library_id)
	{
		...
		if (DB::isError($id)) { trigger_error($rs->getMessage(), E_USER_ERROR); }
	...
	function setMonograph_library_id($library_id) { $this->monograph_library_id   = (!is_numeric($library_id)) ? $this->getLibraryID() : $library_id; }
	function setMultimedia_library_id($library_id) { $this->multimedia_library_id = (!is_numeric($library_id)) ? $this->getLibraryID() : $library_id; }	
}
?>
